...
const createInvoice = (req, res) => {
    ...
    const insertQuery = `
    INSERT INTO invoice (client_name, invoice_link, payable_amount, products, client_id) 
    VALUES (?, ?, ?, ?, ?);
  `;

    connection.query(insertQuery, [client_name, invoice_link, payable_amount, productsJson, clientId], (err, results) => {
        if (err) {
...
const getAllInvoices = (req, res) => {
    const {clientId} = req.params;
    const query = "SELECT invoice.invoice_id, invoice.client_name, invoice.invoice_link, invoice.payable_amount, invoice.products FROM invoice JOIN client ON invoice.client_id = client.client_id where client.client_id = ?;";
